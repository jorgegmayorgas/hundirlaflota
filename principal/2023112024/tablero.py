...
class Tablero:
    ...
    #def muestra_barcos_1(self):
     #   for fila in range(self.dimensiones[0]):
      #      for columna in range(self.dimensiones[1]):
       #         if self.tablero_visible[fila, columna] == 1 :
        #            print("1", end="")
         #       print(" | ", end="")
          #  print("\n" + "-" * (self.dimensiones[0] * 5))
    ...
    def muestra_barcos_1(self):
        for fila in range(self.dimensiones[0]):
            for columna in range(self.dimensiones[1]):
                print(f"fila:{fila} columna:{columna} valor tablero_visible:{self.tablero_oculto[fila, columna]}")
                
    def muestra_barcos_2(self):
        for fila in range(self.dimensiones[0]):
            for columna in range(self.dimensiones[1]):
